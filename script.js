...
function renderResume(data){
  ...
  [document.getElementById('contactInfoMobile'), document.getElementById('contactInfoDesktop')].forEach(node=>{
    ...
    node.innerHTML = (email || phone)
      ? `${phone?`<p><strong>Phone:</strong> <a rel="nofollow" href="tel:${phoneDigits}">${phone}</a></p>`:''}`+
...
// ================= CONTACT TOGGLE (Show contact info) =================
...
// ================= DYNAMIC NAV (custom sections placeholder) =================
function buildNavFromSections(){
  ...
  // if you ever add custom sections back, they will be added here
}
...
// ================= IMAGE MODAL (desktop, single click) =================
...
// ================= FETCH RESUME (index.html) =================
async function fetchResume(){
  try{
    const url = 'assets/resume.json?v=' + Date.now(); // cache-bust
    ...
    const root=document.getElementById('contentRoot');
    if (root) root.innerHTML = '<section class="show"><h2>Error</h2><p>Could not load <code>assets/resume.json</code>. If this is GitHub Pages, confirm the publishing source and that <code>/assets/resume.json</code> exists there.</p></section>';
  }
}
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  wireProfileClicks();
  if (!window.SKIP_AUTO_LOAD) fetchResume();
});
